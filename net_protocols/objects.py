...
from common.utils import base_query, EndPoints
from common.exceptions import (
    CommandNotFoundError,
    ResetModeNotSelectedError,
    ShortPeriodOfTimeToConnectError,
)
...
class LLDP:
    ...
    def get_lldp_from_client() -> Union[str, None]:
        """ Returns info about Client [ interface, portDescr, mac ] """
    ...
    def get_lldp_from_switch() -> Union[str, None]:
        """ Returns info about Switch [ local interface, mac, portDescr ] """
...
class NetPorts:
    ...
    def generate_icmp_packets(host: str) -> None:
        ...
        ping: list[str] = f'ping -i 5 {host}'.split()
...
class ResetButton:
    ...
    def run_commands_context_conn_session(ssh_conn: Any = "") -> Callable:
        ...
        def execute_commands(command: Union[str, None] = None,
                             config_commands: Union[List[str], None] = None) -> Any:
            ...
            raise CommandNotFoundError('Сommand was not passed !')
    ...
    @staticmethod
    def do_reconnect(short_reset: bool = False, long_reset: bool = False,
                     wait_for_reconnect: int = 0) -> None:
        ...
        if short_reset:
            ...
            raise ResetModeNotSelectedError('One of the reset modes is not selected !')

        if wait_for_reconnect >= 120:
            print(f'Connect aborting ... reconnect after {wait_for_reconnect}s Use {mode} !\n')
            sleep(wait_for_reconnect)
        else:
            raise ShortPeriodOfTimeToConnectError('Required time to connect 120 seconds or more !')
...
class SSL:
    ...
    def is_open_https_port(ip: str) -> bool:
        ...
        for string in sp.check_output(cmd).decode('utf-8').split('\n'):
            if 'open' in string:
                return True
        return False
    ...
    def exec_method(method: str) -> bool:
        ...
        return sp.run(commad, check=False, capture_output=True).check_returncode() is None

    @staticmethod
    def start_generate_sslcert() -> bool:
        """
        Wraps for run to a script
        that generates SSL certificates supported by the host
        """
        path: str = f"{getcwd()}/scripts/"
        script: str = 'generate_sslcert.sh'
        if script in sp.check_output(['ls', '-l', f"{path}"]).decode(encoding='utf-8').split():
            command: list[str] = ['bash', f"{path}{script}"]
            sp.run(command, check=True)
            return True
        else:
            raise FileNotFoundError(f"File {script} not found in the current work dir !")
...
class Temperature():
    """ Checking permissible temperature values """
    @staticmethod
    def is_cpu_ok(cpu: float) -> bool:
        """ CPU sensor check """
        lower_limit: float = 50.0
        upper_limit: float = 65.0
        return lower_limit <= cpu <= upper_limit

    @staticmethod
    def is_phy_ok(phy: float) -> bool:
        """ PHY sensor check """
        lower_limit: float = 60.0
        upper_limit: float = 75.0
        return lower_limit <= phy <= upper_limit
